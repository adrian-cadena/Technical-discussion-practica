# Technical-discussion-practica
Este es un ejercicio de la implementación de Technical Discussion para documentación en proyectos.

## Github + Markdown
* Como primer paso utilizaremos GitHub con Markdown que es un tipo de código para implementar comentarios en nuestros proyectos y discusiones técnicas. efectivas.
  * Primero crearemos un nuevo repositorio en Github.
## Gist
* Es una forma de compartir fragmentos de código dentro de los editores de Github.
* Crear un Gist.
  * Primero abrimos una nueva pestaña en el buscador y accedemos con nuestra cuenta de Github a GithubGist.com
  * Damos clic en crear nuevo Gist o en el icono de + para crear un nuevo Gist.
  * Agregamos una descripción corta de nuestro Gist.
  * Le damos un nombre con la extensión del código en el que escribiremos el código, por ejemplo "Holamundo.py" esto sera un hola mundo en python.
  * Una vez creado nuestra sección de código elegimos si lo queremos publico o secreto, en este caso elegimos publico y crear Gis.
## Publicar un Gist en un repositorio.
Para publicar un Gist en un repositorio solo es necesario incluirlo con el Markdow. En la parte superior de cada Gist hay un menu desplegable para compartirlos
seleccionamos la opción "share" y copiamos la dirección, a continuación podemos hacer referencia a este poniendo entre [] el texto que se convertira en un hipervínculo y seguido de este entre () la direccion que copiamos para compartir.

Esta es mi [Gist] (https://gist.github.com/adrian-cadena/9fc153537641dbbc4016093701f217b6)
